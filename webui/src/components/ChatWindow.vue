  ...
  <div class="chat-window">
    ...
    <header class="chat-header" @click="showInfo = true">
      <div class="header-left">
        <img :src="resolveUrl(chat.photo) || DEFAULT_AVATAR" class="header-avatar" @error="handleImgError" />
    ...
    <div class="messages-area" ref="messagesContainer">
      <div v-if="!identityLoaded || (loading && messages.length === 0)" class="loading-msgs">
        <div class="spinner-small"></div>
      </div>

      <div v-else-if="messages.length === 0" class="empty-chat">
        <span class="empty-badge">No messages.</span>
      </div>

      <div v-else v-for="msg in messages" :key="msg.id || msg.ID">
        
        <div v-if="isSystemMessage(msg)" class="system-msg-row">
          ...
          <div class="message-bubble">
            ...
            <div v-if="msg.replyTo || msg.ReplyTo" class="reply-quote-block">
              ...
              <div class="reply-preview-content">
                ...
                <span v-else>{{ getRawText(msg.replyTo || msg.ReplyTo) }}</span>
              </div>
            </div>
            ...
            <div class="message-content">
              <div v-if="msg.photo || msg.Photo" class="photo-wrapper">
                <img 
                  :src="resolveUrl(msg.photo || msg.Photo)" 
                  class="message-image clickable" 
                  @click="openImage(msg.photo || msg.Photo)" 
                  @load="scrollToBottom" 
                />
                <p v-if="getRawText(msg)" class="caption-text">{{ getRawText(msg) }}</p>
              </div>
              <div v-else class="text-content">{{ getRawText(msg) }}</div>
            </div>
    ...
    <footer class="chat-footer">
      <div v-if="replyingToMessage" class="reply-preview-bar">
        <div class="reply-info">
          ...
          <p class="reply-target-text">
            {{ (replyingToMessage.photo || replyingToMessage.Photo) ? '📷 Photo' : getRawText(replyingToMessage) }}
          </p>
        </div>
        <button class="close-reply-btn" @click="cancelReply">✕</button>
      </div>

      <div v-if="selectedFilePreview" class="image-preview-bar">
        <div class="preview-content">
          <img :src="selectedFilePreview" class="preview-thumb">
          <span>Ready to send</span>
        </div>
        <button class="close-reply-btn" @click="clearFileSelection">✕</button>
      </div>

      <div class="input-container">
        <span class="footer-icon camera-btn" @click="triggerFileSelect">📷</span>
        ...
        <span 
          class="footer-icon send-btn" 
          @click="handleSend" 
          :class="{ 'disabled': !newMessage.trim() && !selectedFile }"
        >➤</span>
      </div>
    </footer>
    ...
    <Teleport to="body">
      ...
      <ForwardModal 
        ...
        @close="msgToForward = null" 
        @forwarded="handleForwardedSuccess" 
      />
...
export default {
  ...
  methods: {
    ...
    formatSystemMessage(msg) {
      let text = (msg.text || msg.Text || '').substring(4);

      if (text.startsWith('added ') && text.endsWith(' to the group')) {
        const addedName = text.replace('added ', '').replace(' to the group', '').trim();
        const senderName = msg.sender?.name || msg.Sender?.Name || 'Someone';
        
        const myNameSafe = (this.myName || '').trim().toLowerCase();
        const addedNameSafe = addedName.toLowerCase();

        if (this.isMe(msg)) {
          return `You added ${addedName}`;
        } else if (addedNameSafe === myNameSafe) {
          return `${senderName} added you`;
        } else {
          return `${senderName} added ${addedName}`;
        }
      }

      if (text === 'Group created') {
        if (this.isMe(msg)) return "You created the group";
        const senderName = msg.sender?.name || msg.Sender?.Name || 'Someone';
        return `${senderName} created the group`;
      }
    ...
    async handleSend() {
      ...
      this.sending = true;
      const replyToId = this.replyingToMessage ? (this.replyingToMessage.id || this.replyingToMessage.ID) : null;
      const tempText = text;
      ...
      try {
        ...
        if (tempFile) formData.append('photoFile', tempFile);
        if (replyToId) formData.append('replyToMessageId', replyToId);

        const response = await this.$axios.post(`/conversations/${this.chat.id}/messages`, formData);
    ...
    shouldShowSenderName(msg) {
      return this.chat.type === 'group' && !this.isMe(msg);
    },
    formatTime(dateStr) {
      if (!dateStr) return '';
    ...
    async handleGroupClick(msg, group) {
      if (group.isMine) {
        try {
          await this.$axios.delete(`/conversations/${this.chat.id}/messages/${msg.id || msg.ID}/reactions/${group.myReactionId}`);
          msg.reactions = msg.reactions.filter(r => (r.id || r.ID) !== group.myReactionId);
        } catch (e) {
          console.error(e);
        }
      } else {
        this.addReactionInternal(msg, group.emoji);
      }
    },
    ...
    async handleReactFromModal(emoji) {
      if (this.msgToReact) {
        await this.addReactionInternal(this.msgToReact, emoji);
        this.closeReactionModal();
      }
    },
    async addReactionInternal(msg, emoji) {
      const myId = localStorage.getItem('userId');
      try {
        ...
        const response = await this.$axios.post(`/conversations/${this.chat.id}/messages/${msgId}/reactions`, { emoji });
    ...
    async loadIdentity() {
      ...
      try {
          ...
          localStorage.setItem('userName', this.myName);
      } catch (e) {
          console.error("Error loading identity from API, fallback to localStorage", e);
    ...
    async loadMessages(isBackgroundUpdate = false) {
      if (!this.chat?.id) return;
      if (isBackgroundUpdate && this.msgToReact) return;
      
      this.$axios.post(`/conversations/${this.chat.id}/seen`).catch(() => {});
      
      if (!isBackgroundUpdate && this.messages.length === 0) this.loading = true;
      try {
        const res = await this.$axios.get(`/conversations/${this.chat.id}/messages`);
        const list = res.data || [];
        const orderedList = list.reverse();
        if (this.messages.length > 0) {
          orderedList.forEach(serverMsg => {
            const localMsg = this.messages.find(m => (m.id || m.ID) === (serverMsg.id || serverMsg.ID));
        ...
        this.messages = orderedList;
        if (!isBackgroundUpdate) this.scrollToBottom();
      } catch (e) {} finally {
        this.loading = false;
      }
    },
...
<style scoped>
.system-msg-row { display: flex; justify-content: center; margin: 15px 0; }
.system-badge { background: #202c33; color: #ffd279; padding: 5px 12px; border-radius: 8px; font-size: 0.8rem; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.3); text-transform: uppercase; font-weight: 600; letter-spacing: 0.5px; }
...
.image-preview-bar { background-color: #1d262d; padding: 8px 12px; display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #333; margin: 5px 10px 0 10px; border-radius: 8px; }
.preview-thumb { height: 40px; width: 40px; object-fit: cover; border-radius: 4px; margin-right: 10px; border: 1px solid #00a884; }
.preview-content { display: flex; align-items: center; color: #e9edef; font-size: 0.9rem; }
...
.camera-btn { margin-right: 10px; font-size: 1.5rem; cursor: pointer; color: #8696a0; transition: color 0.2s; }
.camera-btn:hover { color: #00a884; }
.reactions-container { display: flex; flex-wrap: wrap; gap: 5px; margin-top: 6px; margin-bottom: -4px; float: right; width: 100%; justify-content: flex-end; }
.reaction-pill { background: #233138; border: 1px solid #3b4a54; border-radius: 16px; padding: 3px 8px; font-size: 0.85rem; color: #e9edef; cursor: pointer; display: flex; align-items: center; gap: 5px; user-select: none; transition: all 0.2s; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.2); }
.reaction-pill:hover { background: #374248; transform: translateY(-1px); }
.reaction-pill.reaction-mine { background: #005c4b; border-color: #00a884; color: white; }
.emoji-char { font-size: 1rem; line-height: 1; }
.reaction-count { font-size: 0.75rem; font-weight: bold; opacity: 0.9; }
